...
class TestLocalRegistryClientInit:
    ...
    def test_detects_registry_mode(self, temp_registry: Path):
        """Detects registry mode when registry.json exists."""
        client = LocalRegistryClient(str(temp_registry))
        assert client._is_registry is True

    def test_detects_direct_mode(self, temp_plugin_dir: Path):
        """Detects direct mode when no registry.json."""
        client = LocalRegistryClient(str(temp_plugin_dir))
        assert client._is_registry is False
...
class TestLocalRegistryClientGetPackageInfo:
    ...
    def test_gets_package_from_directory(self, temp_plugin_dir: Path):
        """Gets package info from direct directory mode."""
        client = LocalRegistryClient(str(temp_plugin_dir))
...
class TestLocalRegistryClientResolvePackage:
    ...
    def test_resolves_from_directory(self, temp_plugin_dir: Path):
        """Resolves package from direct directory."""
        client = LocalRegistryClient(str(temp_plugin_dir))
...
class TestLocalRegistryClientListPackages:
    ...
    def test_lists_single_package_from_directory(self, temp_plugin_dir: Path):
        """Lists single package from direct directory."""
        client = LocalRegistryClient(str(temp_plugin_dir))
    ...
    def test_returns_empty_for_invalid_directory(self, temp_dir: Path):
        """Returns empty list for directory without package.json."""
        client = LocalRegistryClient(str(temp_dir))
