...
// Deploys:
//   - Storage Account with static website hosting for binary artifacts, install scripts, and website
...
// Note: Static website hosting (index document = index.html, 404 document = index.html)
// is enabled via 'az storage blob service-properties update' in deploy.sh because
// Bicep does not natively expose the staticWebsite property on storage accounts.

@description('Name of the deployed storage account')
output storageAccountName string = storageAccount.name

@description('Static website primary endpoint URL (serves artifacts + website)')
